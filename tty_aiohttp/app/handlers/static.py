...
class StaticResource(web.StaticResource):
    ...
    async def _handle(self, request: web.Request) -> web.StreamResponse:
        ...
        try:
            info = await self.file_info(str(file_path))
        ...
        response.headers[hdrs.CACHE_CONTROL] = "must-revalidate, max-age=0"
        response.headers[hdrs.ETAG] = etag
        response.headers[hdrs.CONTENT_TYPE] = str(ct)
        response.headers[hdrs.CONTENT_ENCODING] = str(encoding)
